...
def get_results(email_list, service, opts, hibp_api_key):
    ...
    for email in email_list:
        ...
        try:
            response = requests.get(url=PWNED_API_URL % (
                service, email, names_only), headers=HEADERS)
            is_pwned = True
            ...
            if response.content:
                ...
                # No results came back for this email.  According to HIBP, this email was not pwned.
                data = None
                is_pwned = False
            ...
            try:
                ...
                if not last_result[PWNEDINDEX]:
                    if service == BREACHED:
                        ...
                        print(
                            "Email address %s was not found in any pastes.  Yay!" % (email))
                else:
                    print("\n%s pwned!\n==========" % (email))
...
def write_results_to_file(filename, results, opts):
    ...
    BREACH_HEADER = ("Email Address", "Is Pwned", "Name", "Title", "Domain", "Breach Date", "Added Date", "Modified Date", "Pwn Count",
                     "Description", "Logo Path", "Data Classes", "Is Verified", "Is Fabricated", "Is Sensitive", "Is Retired", "Is SpamList")
